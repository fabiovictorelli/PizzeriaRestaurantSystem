<?PHP 
   ...
   MostraCabec("Listagem de Produtos por Data");
   MostraMenuAdmProdutos();

   $conn = pg_Connect("localhost", "5432", "", "", "victorelli") or die("Não foi possível conectar ao banco de dados"); 
   $result = pg_Exec($conn, "Select * from produtos ORDER BY cod_tiposprodutos,numero;");
   $num = pg_NumRows($result);

   if($num == 0) {
      MostraErro("Não existem produtos cadastradas!\n<BR>Você deve cadastrar um produto primeiro !\n");
   }

   echo "<table  border=\"1\"  cellpadding=\"1\" cellspacing=\"1\">";
   echo "<tr align=\"center\"> <td>TIPO</td><td>NUMERO</td><td>NOME</td><td>DESCRICAO</td></tr>";
   for($i = 0; $i < $num; $i++) {
